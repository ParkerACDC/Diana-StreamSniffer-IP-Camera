...
def check_host_alive(ip):
    # ping detect if the host is down or blocked by ap isolation
    print(f"\n{C}Performing host discovery on {ip}...{W}")
    
    # determine which OS for correct ping command
    ...
    try:
        ...
        if output.returncode == 0:
            print(f"  {G}✅ Host is alive (Responded to ICMP Ping){W}")
            return True
        else:
            print(f"  {R}🚨 [WARNING] Aww snap! Host is unreachable x_x {W}")
            print(f"  {C}    Reason:{W}")
            print(f"  {C}    (1) The camera is off or not connected to Wi-Fi{W}")
            print(f"  {C}    (2) AP isolation / Guest Network mode is active on the router{W}")
            return False
    except Exception as e:
        print(f"  {Y}⚠️ Ping failed {W}")
        return True
...
def main():
    try:
        ...
        #Host discovery & isolation check
        is_alive = check_host_alive(target_ip)
        
        if not is_alive:
            force_scan = input(f"\n{Y}[?] Do you want to force the port scan anyway? (y/n): {W}").strip().lower()
            if force_scan != 'y':
                print(f"\n{C}Scan aborted by user.{W}")
                return
            else:
                print(f"\n{Y}Forcing scan despite failed ping...{W}")
        ...
        if not open_ports:
            ...
            if not is_alive:
                print(f"{R}AP Isolation is active or the host is offline.{W}")
            else:
                print(f"{C}The host is alive, but all scanned ports are closed/filtered by a firewall.{W}")
            return
